...
(function () {
	...
	const register = editor => {
		...
		function validWordToken(word) {
			...
			if (/\s/.test(word)) {							// is space
				return false;
			}
			if (/[\:\.\@\/\\]/.test(word)) { 				// is punctuation
				return false;
			}
			if (/^\d+$/.test(word) || word.length == 1) { 	// is number
				return false;
			}
			if (isCorrectWord(word)) {						// is correct (spell checked)
				return false;
			}
			return true;
		...
		const getWords = (editor) => {
			...
			for (var i = 0; i < matches.length; i++) {
				...
				if (!uniqueWords[word] && validWordToken(word)
		...
		function MarkAllTypos(body) {
			...
			for (var i = 0; i < allTextNodes.length; i++) {// NOTE: i here is for each line
